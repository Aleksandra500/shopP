...
const cartSlice = createSlice({
    ...
    reducers: {
        saveInCartAction: (state, action) => {
            ...
            if (findIndex === null) {
                copyCart.push({ ...action.payload, count: 1, cartTotal: parseFloat(action.payload.price) });
                state.totalProduct++;
                state.totalPrice = state.totalPrice + Math.floor(action.payload.price);
            } else {
...
function subTotal(arrayCart) {
    return arrayCart.reduce((acc, current) => {
        return acc + parseFloat(current.cartTotal);
    }, 0);
}
